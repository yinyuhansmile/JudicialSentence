...
class RCNN(object):
    def __init__(self, W_embedding, settings):
        ...
        with tf.variable_scope('rcnn_text'):
            output_title = self.rcnn_inference(self._X_inputs, self.jieba_len)

        # with tf.variable_scope('rcnn_content'):
        #     output_content = self.rcnn_inference(self._X2_inputs, self.content_len)

        with tf.variable_scope('fc-bn-layer'):
            output = tf.concat([output_title], axis=1)
            W_fc = self.weight_variable([self.n_filter_total, self.fc_hidden_size],
                                        name='Weight_fc')
        ...
        with tf.name_scope("accuracy"):
            correct_predictions = tf.equal(self.predictions, tf.argmax(self._y_inputs,
                                                                       1))  # 罪名法条刑期分类  # correct_predictions = tf.equal(self.predictions, self._y_inputs) #刑期回归预测
...
def test():
    ...
    print('Begin testing...')
    settings = Settings()
    W_embedding = np.random.randn(50, 300)
    ...
    with tf.Session(config=config) as sess:
        ...
        loss_list = list()
        for i in range(100):
            X1_batch = np.zeros((batch_size, 400), dtype=float)
            # X2_batch = np.zeros((batch_size, 150), dtype=float)
            y_batch = np.zeros((batch_size, 202), dtype=int)
