...
def gettimeclass(label):
    if label == -2:
        return 0
    elif label == -1:
        return 1
    elif label < 25:#0-2年刑期按月划分
        return label+2
    elif label < 27:
        return 27
    elif label < 29:
...
def timelabelTtransform():
    validtimelabel = list(np.load(jieba_valid_path+'valid_time_value.npy'))
    testtimelabel = list(np.load(jieba_test_path+'test_time_value.npy'))
    traintimelabel = list(np.load(jieba_train_path+'train_time_value.npy'))
    validtimeclass = []
    count = len(validtimelabel)
    index = 0
    print(count)
    for valid in validtimelabel:
        a = gettimeclass(valid)
        validtimeclass.append(a)
        index+=1
    print(index)
    np.save(jieba_valid_path+'valid_time_label.npy',np.asarray(validtimeclass))
    
    testtimeclass = []
    count = len(testtimelabel)
    index = 0
    print(count)
    for test in testtimelabel:
        a = gettimeclass(test)
        testtimeclass.append(a)
        index+=1
    print(index)
    np.save(jieba_test_path+'test_time_label.npy',np.asarray(testtimeclass))
    
    traintimeclass = []
    count = len(traintimelabel)
    index = 0
    print(count)
    for train in traintimelabel:
        a = gettimeclass(train)
        traintimeclass.append(a)
        index+=1
    print(index)
    np.save(jieba_train_path+'train_time_label.npy',np.asarray(traintimeclass))
...
#     timelabelTtransform()
    validtimelabel = list(np.load(jieba_train_path+'train_time_label.npy'))
    validdict = {}
    for a in validtimelabel:
        if a in validdict.keys():
            validdict[a] += 1
        else :
            validdict[a] = 1
    print(validdict.keys())
    print(validdict)
    index = 0
    for a in validdict.keys():
        index += validdict[a]
    print(index)
#     testtimelabel = list(np.load(jieba_test_path+'test_time_label.npy'))
#     traintimelabel = list(np.load(jieba_train_path+'train_time_label.npy'))
#     print(validtimelabel[:10])
#     print(testtimelabel[:20])
#     print(traintimelabel[:60])
#     print(min(validtimelabel))
#     print(max(validtimelabel))
#     print(min(testtimelabel))
#     print(max(testtimelabel))
#     print(min(traintimelabel))
#     print(max(traintimelabel))
