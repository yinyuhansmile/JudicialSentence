...
marked_labels_list = np.load('accu_labels.npy')
scores_path = './'
...
def get_score(score):
    ...
    for predictlabel in score:
        xitem = np.argwhere(predictlabel > -2.7).flatten()
...
if __name__=='__main__':  
    weights = [ 1.0,0.8,0.5,0.45,0.4,0.3,
 -0.1,-0.56]
    ...
    logging.info('init weights= %s'%str(weights))
    print('local  f1=%g;' % (f1))
    logging.info('local  f1=%g;'% (f1))
    time0 = time.time()
    starttime = time0
    last_f1 = f1
    best_f1 = f1
