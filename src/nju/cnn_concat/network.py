...
#             self._X2_inputs = tf.placeholder(tf.int64, [None, self.thulac_len], name='X2_inputs')
        ...
        with tf.variable_scope('cnn_text'):
            output_jieba = self.cnn_inference(self._X_inputs, self.jieba_len)

#         with tf.variable_scope('hcnn_content'):
#             output_content = self.cnn_inference(self._X2_inputs, self.thulac_len)

        with tf.variable_scope('fc-bn-layer'):
            output = tf.concat([output_jieba], axis=1)
...
def test():
    ...
    print('Begin testing...')
    settings = Settings()
    W_embedding = np.random.randn(200000, 300)
...
if __name__ == '__main__':
    lists = [0,2,-2,6,10,7,9]
    array111 = np.array(lists)
    a = array111.argsort()[-1:-1:-1]
    print(a)
#     test()
#     naaa = np.zeros((193393,300))
#     print(naaa.shape)
